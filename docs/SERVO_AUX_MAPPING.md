...
## 4.3 ⚠ OPEN: กล่องใน SITL ไม่ได้ตกตรงจุดที่ปล่อย (2026-08-16)

เกณฑ์ใหม่จากผู้ใช้ (2026-08-15): **"err ห้ามเกินขอบวงกลมของ ArUco"** ⇒ ไข่ต้องตก
ในวงแหวนดำ ⌀750 mm = ห่างจากกลางแพดไม่เกิน **0.375 m** (ถ้าเอาแบบทั้งกล่องต้องอยู่
ในวง: 0.375 − ครึ่งเส้นทแยงหน้าตัดกล่อง 0.16×0.065 m = **0.288 m**)

⚠ **เกณฑ์นี้วัดคนละอย่างกับเลข `err` ที่เรารายงานกันมาตลอด** — `err` คือระยะที่
...
**และตอนพยายามวัดครั้งแรกก็เจอความไม่ตรงกันทันที** (ไฟลท์ 2026-08-16, ลม 10 m/s):

| | ENU |
|---|---|
| จุดที่ audit บันทึกว่าปล่อย | (−10.44, 31.48) — ห่างกลางแพด id 1 **0.12 m** ✓ |
| กล่อง `cargo_payload_3` ที่ตกจริง | (−17.01, 28.17) — **ห่างจุดปล่อย 7.35 m** ✗ |

กล่องนิ่งสนิท (สุ่มอ่าน 3 ครั้ง ค่าเท่ากันทศนิยมครบ) ⇒ ไม่ใช่ลมพัด และทิศที่มัน
ไปอยู่เป็นทิศ **ทวนลม** ด้วย จุดที่มันไปตกคือจุดที่โดรนบินผ่านตอน **t=51.6 s
ระหว่างสวีป** (ห่าง 1.12 m, ที่ความสูง 4 m) — ไม่ใช่ตอนปล่อยที่ t=118.3 s ส่วน log
ของ bridge มีบรรทัด shed บรรทัดเดียว เวลาตรงกับ RELEASE

**ยังไม่รู้สาเหตุ** สมมติฐานที่ยังไม่ได้ตัดออก: กล่องหลุดตั้งแต่ระหว่างสวีปด้วยกลไก
อื่น แล้ว detach ของ bridge ตอนปล่อยเป็น no-op / DetachableJoint หลุดเองจากแรงตอน
บิน / กล่องถูกดีดตอนไต่ขึ้นจากแพด

⚠ **ผลกระทบ: หลักฐาน "กล่องตกบนแพด" จาก SITL ทั้งหมดยังเชื่อไม่ได้จนกว่าจะไล่จบ**
และเกณฑ์ใหม่ของผู้ใช้วัดที่กล่องพอดี **วิธีไล่ที่ถูกที่สุด**: บินใหม่แล้วอ่าน
`tools/box_truth.py` ทุก ~10 วิระหว่างบิน จะได้รู้ว่ากล่องหลุดวินาทีไหน แทนที่จะ
อ่านครั้งเดียวหลังจบ
