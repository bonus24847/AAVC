...
\code{BAT1_N_CELLS} / \code{V_CHARGED} / \code{V_EMPTY} / \code{R_INTERNAL} & 6 / 4.18 / \hl{3.65} / \hl{0.004} & re-fit จาก ULog แพ็คขนานคืน 29 ส.ค. (sag 1.3–1.5 V @ 54 A = 0.004 $\Omega$/เซลล์; 3.77 เรียก ``หมด'' ทั้งที่เหลือจริง ~30\%) — เกจใต้โหลดจะอ่านเท่าตอนพัก; พื้น 30/20/15/10\% = สำรองจริง ~35/28/22/15\%\\
...
\code{MPC_THR_HOVER} & \hl{0.70} & วัดจริงกับแพ็คขนานที่ Bang Bo คืน 29 ส.ค.: motor mean 0.69–0.72 ใน 3 ไฟลท์ (เขียนบอร์ด + อ่านกลับแล้ว 23:45)\\
...
\item \who{ENG} \textbf{หลัง deploy: รีสตาร์ท infra ให้โหลดโค้ดใหม่} — \code{ssh drone@10.42.0.1 'pkill -f \"camera\_grabbe[r].py\"; pkill -f \"status\_beacon.p[y]\"; bash \textasciitilde/mission/cm4/start\_infra.sh'} (deploy เปลี่ยนไฟล์บนดิสก์ แต่ไม่รีสตาร์ท process ที่รันอยู่ — \code{--check} จะขึ้น MATCH ทั้งที่กล้อง/beacon ยังรันโค้ดเก่า)
\item \who{ENG} \hl{\textbf{DEPLOY ก่อนอย่างอื่น}} — คืน 30 ส.ค. 02:40 แก้บั๊ก 7 จุด (บันได/ปล่อยไข่/รัศมีรับ pad/แบตช่วง decode) แต่ CM4 ดับก่อน: \code{bash cm4/deploy.sh drone@10.42.0.1 --repo \textasciitilde/Desktop/aavc-comp} แล้ว \code{--check} ต้องขึ้น \textbf{MD5 MATCH} ก่อนทำข้อถัดไป
\item \who{ENG} \hl{\textbf{ไฟเลี้ยง CM4 เต็ม + พอ > 30 นาที ก่อน GO}} — CM4 กินไฟจากแหล่งแยก ไม่ใช่แพ็คบิน: บางบ่อไฟลท์ 5 (30 ส.ค. 00:41) ไฟ CM4 หมดกลางอากาศ → FC ไม่ได้รับ goto ต่อ → โดรน\textbf{ลอยนิ่งที่ setpoint สุดท้ายจนนักบินลง} (PX4 ไม่มี failsafe สำหรับ companion หาย เพราะวิทยุ console นับเป็น GCS อยู่) · ถ้าเกิดกลางภารกิจ: console ขึ้นแถบแดง ``CM4 เงียบ > 15 วิ'' → นักบิน POSCTL ลงทันที → เปลี่ยน/ชาร์จไฟ CM4 → restart stack → recovery flight
\item \who{ENG} \hl{\textbf{กล้องยังค้างโหมดกลางคืน (Bang Bo) — console คืนค่าให้เองตอนต่อ CM4ครั้งแรก (aavc-gcs a275430) แต่ต้อง VERIFY}}: หลัง console ขึ้น ``CM4 กล้อง+beacon สตาร์ทแล้ว'' → \code{ssh drone@10.42.0.1 'ps -o args= -p \$(pgrep -f camera_grabbe[r].py)'} ต้องมี \code{--ae-highlight} และ \textbf{ไม่มี} \code{--gain 128}; ถ้ายังมี: \code{rm -f ~/.aavc_night_cam; pkill -f \"camera_grabbe[r].py\"; bash ~/mission/cm4/start_infra.sh} แล้วดูใหม่ (ถ้าไม่คืน: กลางแดดภาพขาวโพลน decode 0 ทั้งวัน) · แล้ว \code{cm4/deploy.sh --check} ต้อง MATCH (deploy ถ้าไม่)
...
\item \who{OP} \code{MPC_THR_HOVER = 0.70} บนบอร์ด (BOARD ผ่าน; วัดจริง 0.69–0.72 คืน 29 ส.ค.)
...
\item \hl{วางไม่ตรง ดีกว่าไม่วาง} (operator 29 ส.ค.): rung 2 m lock ไม่ทันแต่ยังเห็น pad ภายใน 0.45 m → ระบบ \textbf{ลงเลย} (audit \code{LAND GATE relaxed}); ครั้งที่สอง (retry) ลงถ้าเห็น pad ภายใน 1.0 m · ยังไม่ลงถ้า \textbf{ไม่เห็น pad ที่ 2 m} หรือ \textbf{ไม่เคย decode id ที่ถูกต้อง} (ผิด pad แย่กว่าไม่วาง)
