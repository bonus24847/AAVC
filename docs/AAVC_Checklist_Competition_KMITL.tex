...
\textbf{อากาศยาน} & EFT X6100 hexacopter · AUW 7.2 kg & \textbf{FC / Companion} & Pixhawk 6X (PX4 1.17) + Raspberry Pi CM4\\
\textbf{Profile} & \hl{\code{competition} (\code{kmitl_config.yaml})} & \textbf{Envelope} & transit \textbf{20 m} · ceiling 20 · floor 10 · sweep 12 m\\
\textbf{แบตเตอรี่} & 6S 17000 mAh (เต็ม 25.1 V) + สำรอง 15000 & \textbf{Window} & \textbf{20 นาที} นับจาก GO แรก · 4 ไข่ ใน 1 เที่ยว\\
\end{tabular}
...
\begin{cl}
\item \who{ENG} \textbf{ชาร์จแบต 17000 และ 15000 ให้เต็ม} (25.1 V = 4.18 V/cell) แล้ว \hl{ถอดแพ็คออกจากเครื่อง} — เสียบค้างกิน $\sim$0.76 A ตลอดเวลา (เคยหาย 3.8 Ah ใน 5 ชม.)
...
\item \who{OP} \textbf{geofence + ceiling}: อธิบายได้ว่า \code{GF_ACTION = 3 (Return)} verify แล้ว · เพดาน 20 m ที่ companion บน AGL ที่ latch ตอน arm · \code{GF_MAX_VER_DIST = 50} เป็นตาข่ายใหญ่ (home ของ PX4 เลื่อนกลางอากาศ)
\item \who{OP} \textbf{แบต}: เกจ voltage-only อ่าน conservative · floor RTH 30\% / LAND 20\% · energy gate ปฏิเสธไม่เริ่มถ้าไม่พอ
...
\code{BAT1_N_CELLS} / \code{V_CHARGED} / \code{V_EMPTY} & 6 / 4.18 / 3.77 & endpoints ของแพ็ค 17000 semi-solid\\
...
\code{MPC_THR_HOVER} & 0.58 & hover ของลำนี้ (bench 21 ส.ค.)\\
\code{RTL_RETURN_ALT} / \code{GF_MAX_VER_DIST} / \code{GF_ACTION} & 19.5 / 50 / 3 & Return ที่เพดาน · ตาข่ายใหญ่ · \textbf{3 = Return} (ห้าม 2 = Hold)\\
...
\begin{cl}
\item \who{ENG} \textbf{เสียบแบต 17000} (ที่จุดปล่อยเท่านั้น) · จดแรงดัน: \underline{\hspace{1.5cm}} V (ต้อง $\geq$ 24.6 V ≈ 90\%)
...
\item \who{OP} ดูบรรทัดเริ่มของ orchestrator: \hl{\code{transit 3 pts @ 20 m, sweep 5 legs @ 12 m}} — ถ้าเป็น 9 m / 10 m = profile ผิด (practice) \hlr{หยุด แก้ก่อน}
\end{cl}
...
\begin{cl}
\item \who{LEAD} รับ \textbf{4 id} จากกรรมการ · จด: \underline{\hspace{0.8cm}} \underline{\hspace{0.8cm}} \underline{\hspace{0.8cm}} \underline{\hspace{0.8cm}} · ถามว่า \textbf{เวลาเริ่มนับเมื่อไร}
\item \who{OP} เลือก id ทั้ง 4 ตามลำดับ (mission queue) · บันทึก → GCS โชว์ \code{assigned_id_queue} ตรง
\item \who{ENG} \textbf{โหลดไข่ในกล่องหัวใจ 4 ใบ}: delivery 1 → latch AUX4 (FL) · 2 → AUX1 (RR) · 3 → AUX2 (FR) · 4 → AUX3 (RL) — ลำดับปล่อยจริงระบบเลือกตาม route — \hl{ทุก latch ต้องมีไข่}
...
\begin{cl}
\item \who{OP} ทุก 1 นาที: \textbf{เวลาเหลือ · แบต\% · เฟส · pad ที่เจอ/ส่งแล้ว} — ไข่ใบที่ 4 ต้องเริ่มลงเมื่อเหลือ \textbf{$\geq$ 300 s} (gate ปฏิเสธเองถ้าไม่พอ)
...
\item \who{OP} ถ้าบินซ้ำ: \textbf{restart stack} (icon COMP ใหม่) — บินซ้ำโดยไม่ restart = state/กล่องเก่าค้าง
\end{cl}

% ===================== 10 TRIAL SLOT PLAN =====================
\section{แผน trial slot 28 ส.ค. (13:00–18:00, 15 นาที/slot)}
\begin{cl}
\item \textbf{Slot 1 — ภารกิจเต็มบนสนามจริง}: profile competition · เก็บ: decode ที่ 12 m ได้ไหม (ต้องเห็น \code{target_confirmed} ครบ 6) · AGL นิ่ง · transit 3/3 · เวลา sweep จริง vs 407 s
\item ถ้า decode ไม่ได้ที่ 12 m: ดู \code{AE hi=} (ขาวต้อง 190–225) → ปรับ \code{--ae-max-100us} / \code{CAM_EXPOSURE} แล้วใช้ slot 2 — \hlr{ห้ามลด sweep ต่ำกว่า 12 m ถ้ายังไม่รู้สาเหตุ} (floor 10 m คือเส้นกติกา)
\item \textbf{Slot 2 — ซ้อมเต็มพร้อมไข่ซ้อม + จับเวลา 20 นาที} = dress rehearsal (G8) · ซ้อม POSCTL takeover 1 ครั้งตอนจบ
\item หลัง trial: ประชุม 10 นาที — อะไรจะเปลี่ยนก่อนพรุ่งนี้ (ถ้าไม่จำเป็น \textbf{ไม่เปลี่ยน})
\end{cl}
...
OFFBOARD สลับไม่ติด & ไม่มี orchestrator ป้อน setpoint & ต้องกดปุ่ม Up ก่อนเสมอ; ดู \code{RC-GO — staged}\\
...
{\small\color{gray}AAVC 2026 · Team AeroOptix (KMUTNB) · competition @ IAAI KMITL · rev. 2026-08-27 · แหล่ง: กติกา V1.3, CLAUDE.md §0–§8, tools/preflight\_params.py}
\end{document}
