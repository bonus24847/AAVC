...
\item \who{ENG} \hl{\textbf{คืนค่ากล้องจากโหมดกลางคืน (Bang Bo) ก่อนทุกอย่าง}}: \code{ssh drone@10.42.0.1} → \code{rm -f ~/.aavc_night_cam} · \code{v4l2-ctl -d /dev/v4l/by-id/usb-Generic_USB_Camera_200901010001-video-index0 -c exposure_dynamic_framerate=0} · \code{pkill -f \"camera_grabbe[r].py\"} · \code{bash ~/mission/cm4/start_infra.sh} → argv ของ grabber ต้องมี \code{--ae-highlight} และ \textbf{ไม่มี} \code{--gain 128} (ถ้าไม่คืน: กลางแดดภาพขาวโพลน decode 0 ทั้งวัน)
