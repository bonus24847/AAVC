...
**2026-08-28 (หลังไฟลท์ 17:28 ที่ KMITL):** หลังปล่อยแล้ว **latch เปิดค้าง** — ไม่ส่ง
พัลส์ปิด (−0.8) กลับอีก 0.6 วิถัดมาเหมือนเดิม (`connection.drop_servo_relatch: false`;
`true` = พฤติกรรมเก่า release → 0.6 s → hold). บอร์ดปิดทุกพิน AUX เองตอน DISARM
(`PWM_AUX_DISn = 1100`) จึงปิดกลับให้ทีมเติมไข่ที่ L&R โดยอัตโนมัติ.
...
**วัดบนลำจริง 2026-08-28 (IMG_6457, อาจารย์/ทีมวัด):** latch หน้า↔ท้าย (4↔3) **18.5 cm**,
ซ้าย↔ขวา (3↔1) **6.6 cm**, มุมตรงตามตารางล่าง (4 หน้า-ซ้าย · 2 หน้า-ขวา · 3 ท้าย-ซ้าย ·
1 ท้าย-ขวา, แท่นเป็นกากบาทกลางลำ) — ตารางสเปก 20 × 7 cm ต่างจากของจริง < 1 cm
จึง**ไม่ต้องใส่ offset** ในตัวเล็ง; ระยะห่างไข่จาก CG จริง ≤ 0.10 m.
