...
- **KMUTNB สนามซ้อม (เพดาน 10 m)** — repo นี้ (⚠ ป้ายใน `missions.yaml` ของ
  aavc-gcs ยังเขียน "5 m" — ยกเป็น 10 m แล้ว operator 2026-08-18; แก้ป้ายด้วย)
...
⚠ **บนเครื่องจริง dropdown ถูกล็อก** (ตั้งแต่ 2026-08-14): console ที่เปิดด้วย
`launch_gcs_real.sh` ปุ่ม 🚀 ผูกกับ **ssh ไป CM4 ของ mission นี้** ส่วน template
ในรายการเป็นคำสั่งฝั่ง SIM บนโน้ตบุ๊ก — ถ้าปล่อยให้สลับ ปุ่มจะกลายเป็นคำสั่ง
SIM เงียบ ๆ ทั้งที่ผู้ใช้คิดว่ากำลังสั่งโดรน จึงปฏิเสธทั้งฝั่ง UI และ server
(ขึ้นเหตุผลบนหน้าจอ) **การสลับ mission ใช้ได้เฉพาะ SIM**; จะบิน mission อื่นบน
เครื่องจริงต้อง (1) deploy repo นั้นขึ้น CM4 (2) เปิด console ใหม่ด้วย
...
| 🚀 up ขึ้นโดรน (ป้าย [REAL] โผล่เฉพาะ console เครื่องจริง) | ✔ spawn `sitl/run_mission.sh` บนเครื่องนี้ | ✔ ssh ไปสั่ง orchestrator บน **CM4** |
...
`/tmp/aavc_nadir.jpg` — สิ่งที่ detector เห็นจริง ๆ; SIM-only, จอ GCS ไม่มีแผงกล้อง)
...
3. **เปิด console บนโน้ตบุ๊ก — คำสั่งเดียว**:
   ```bash
   cm4/launch_gcs_real.sh aavc@<ip-cm4>
   ```
   สคริปต์นี้ตั้งค่า GCS ให้ครบทุกตัว (จะได้ไม่ต้องพิมพ์เองหน้างาน) ค่าที่มันตั้งให้คือ:

   | ค่า | ความหมาย |
   |---|---|
   | `--field gcs/kmutnb_field.yaml` | เส้น geofence/search/transit ของสนามบนแผนที่ |
   | `--captures captures` | โฟลเดอร์ผลฝั่งโน้ตบุ๊ก (โหมด SIM เขียนที่นี่; โหมด REAL ข้อมูลมาทางวิทยุ) |
   ...
   ⚠ **`status_sync` ถูกถอดออกจาก launcher ตัวจริงเมื่อ 2026-08-18** (คำสั่ง
   operator: "เอาไหนที่ใช้วิทยุไม่ได้ เอาออกเลย") คอนโซลเครื่องจริง**อ่านทุกอย่าง
   จากวิทยุ NOMAD อย่างเดียว** ผ่าน `cm4/status_beacon.py` ที่ยิง STATUSTEXT —
   เฟส, จำนวนที่ส่งแล้ว, พิกัด pad ที่ยืนยัน/เพิ่งเห็น, สาเหตุกลับบ้าน, สุขภาพกล้อง
   ที่วัดข้างกล้องบน CM4 เอง ไม่ใช่จากอายุไฟล์ที่ WiFi ดึงมา
   สิ่งที่ยังวิ่งบน ssh/WiFi มีสามอย่าง และทั้งสามทำงาน**ตอนอยู่ที่จุดปล่อยเท่านั้น**:
   ปุ่ม 🚀 (stage), การยกกล้อง+beacon อัตโนมัติเมื่อ console เจอ CM4
   (`cm4/start_infra.sh` — ชิปกล้องเขียวโดยไม่ต้อง stage), และการดึง**เส้นทาง
   mission** มาวาดบนแผนที่ครั้งเดียวตอน stage (2026-08-22 — วิทยุแบก route ไม่ไหว
   เพราะ STATUSTEXT กว้าง 50 ตัวอักษร) ทั้งสามอย่างขาดได้โดยไม่กระทบการบิน
   ⚠ ถ้าจอเงียบ ให้ดูป้าย **📻 ขาด N วิ** ที่มุมความคืบหน้าก่อน — มันแยกให้ว่า
   "ภารกิจหยุด" กับ "วิทยุขาด" คนละเรื่องกัน
...
3. **สลับเข้า OFFBOARD = ปล่อยเครื่อง**: orchestrator เห็น armed+OFFBOARD
   แล้วรับช่วงใน ~0.2 วิ (takeoff → transit → …). นาฬิกาของ **ซอฟต์แวร์**
   เริ่มนับที่จังหวะสลับสวิตช์ (`state.start_window()`) — **แต่ ⚠ นาฬิกา 20 นาที
   ของ *กรรมการ* เริ่มตั้งแต่หัวหน้าทีมแจ้ง "ระบบพร้อม" หรืออัตโนมัติที่ T+5 นาที
   (กติกา p.10-11)** ไม่ใช่ตอนสลับสวิตช์ ⇒ **อย่าถ่วงเวลาช่วง "แจ้งพร้อม → สลับ
   OFFBOARD"** เพราะทุกวินาทีตรงนั้นคือ window ที่กรรมการนับไปแล้ว (ซอฟต์แวร์
   ไม่เห็นส่วนนี้)
